        ...
        if ($result = mysql_query($sql)) {
            if (mysql_num_rows($result)) {
                ...
                while (list($id, $name, $mod, $ts1, $ts2) = mysql_fetch_array($result)) {
                    ...
                    print "<a href='".$_SERVER["PHP_SELF"]."?page=".$get_vars["page"]."&menu_id=".$get_vars["menu_id"]."&consult_id=".$get_vars["consult_id"]."&ptmenu=".$get_vars["ptmenu"]."&module=$mod&request_id=$id#$mod' title='".INSTR_CLICK_VIEW_RESULTS."'>$name</a> ";
