...
// Ver:		$Id: jpgraph_line.php,v 1.28 2002/06/15 13:20:24 aditus Exp $
//
...
// PUBLIC METHODS	
	...
	$this->csimareas="";
	$np = count($cord)/$factor-$adjust;
	for($i=$adjust; $i<$np; $i+=2) {
	  if( is_numeric($this->coords[0][($i-$adjust)/2]) ) {
	    $xt=$cord[$i*$factor];
	    $yt=$cord[$i*$factor+1];
        if( !empty($this->csimtargets[$i/2]) ) {
	      $this->mark->SetCSIMTarget($this->csimtargets[$i/2]);
	      $this->mark->SetCSIMAlt($this->csimalts[$i/2]);
	      $this->mark->SetCSIMAltVal($this->coords[0][($i-$adjust)/2]);
	    }
