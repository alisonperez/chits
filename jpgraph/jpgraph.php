...
// Ver:		$Id: jpgraph.php,v 1.116 2002/09/15 19:13:14 aditus Exp $
//
...
DEFINE("TTF_DIR","/usr/X11R6/lib/X11/fonts/drakfont/ttf/");
...
//
if( USE_LIBRARY_GD2 && function_exists('imagecopyresampled')) {
    $GLOBALS['gd2'] = true;
    $GLOBALS['copyfunc'] = 'imagecopyresampled';
} elseif(!USE_LIBRARY_GD2 && function_exists('imagecopyresized')) {
...
// PRIVATE METHODS	
	...
	// Stroke title
	$y = 4 + $this->title->margin; $margin=3;
...
class Image {
	...
	if( $this->font_family <= FF_FONT2+1 ) {
	    ...
	    $m=0;
	    for( $i=0; $i < $n; ++$i ) {
		$bbox = ImageTTFBBox($this->font_size,$angle,$file,$tmp[$i]);
		$mm =  abs($bbox[2]) + abs($bbox[0]);
		if( $mm > $m ) 
		    $m = $mm;
	    }
	    // ImageTTFBBox seems to be buggy so that depending on the exact
	    // characters the width is reported as a few pixels to short. We
	    // therefore always add the width of a "thin" character to
	    // compensate
	    $bbox = ImageTTFBBox($this->font_size,$angle,$file,'l');
	    return $m + ($bbox[2]-$bbox[0])/2;
	}
    }
    ...
    function StrokeBoxedText($x,$y,$txt,$dir=0,$fcolor="white",$bcolor="black",
			     ...
			     $xmarg=6,$ymarg=2,$cornerradius=0,$dropwidth=3) {
	...
	if( $shadowcolor ) {
	    ...
	    //$this->ShadowRectangle($x,$y,$x+$width+4,$y+$height+2,$fcolor,2,$shadowcolor);
	    $this->FilledRoundedRectangle($x+$dropwidth,$y+$dropwidth,
					  $x+$width+$dropwidth,$y+$height+$dropwidth,
					  $cornerradius);
	    $this->PopColor();
	    $this->PushColor($fcolor);
	    $this->FilledRoundedRectangle($x,$y,$x+$width,$y+$height,$cornerradius);		
	    $this->PopColor();
	    $this->PushColor($bcolor);
	    $this->RoundedRectangle($x,$y,$x+$width,$y+$height,$cornerradius);
	    $this->PopColor();
	}
	else {
	    if( $fcolor ) {
	...
	$this->StrokeText($x+$xmarg+1, $y+$ymarg, $txt, $dir, $paragraph_align);
    ...
    function _StrokeTTF($x,$y,$txt,$dir=0,$paragraph_align="left") {
	$file = $this->ttf->File($this->font_family,$this->font_style);
	$angle=$dir;
	if( !ereg("\n",$txt) ) {
	    $bbox=ImageTTFBBox($this->font_size,$angle,$file,$txt);
	    ...
	    $standardbox=ImageTTFBBox($this->font_size,$angle,$file,'Gg');
	    $yadj = $standardbox[1];
	    $xadj = $standardbox[0];
	    for($i=0; $i < $nl; ++$i) {
		...
		ImageTTFText ($this->img, $this->font_size, $angle, $xl, $yl-($h-$fh)+$fh*$i, 
			      $this->current_color,$file,$tmp[$i]); 
	...
	if( $this->font_family >= FF_FONT0 && $this->font_family <= FF_FONT2+1) {	
	    $this->_StrokeBuiltinFont($x,$y,$txt,$dir,$paragraph_align);
	}
	elseif($this->font_family >= FF_COURIER && $this->font_family <= FF_BOOK)  { 
	    $this->_StrokeTTF($x,$y,$txt,$dir,$paragraph_align);
	}
	else
...
class RotImage extends Image {
    ...
    function Rotate($x,$y) {
	// Optimization. Ignore rotation and translation if Angle==0
	if( $this->a == 0 ) {
	    return array($x,$y);
	}
	else {
...
class Legend {
    ...
    var $txtcol=array();
    var $mark_abs_size=10,$xmargin=10,$ymargin=3,$shadow_width=2;
...
// PUBLIC METHODS	
	...
	foreach($this->txtcol as $p) {
		...
		$mtw+=$aImg->GetTextWidth($p[0])+$this->mark_abs_size+2*$this->xmargin;
	}
	...
	if( $this->layout==LEGEND_VERT )
	    ...
	    $abs_width=$mtw+2*$this->xmargin;//+2*$this->mark_abs_size+2*$this->xmargin;
	...
	$y1=$yp + $this->ymargin; //+$aImg->GetFontHeight();
	...
	foreach($this->txtcol as $p) {
	    ...
	    $aImg->SetFont($this->font_family,$this->font_style,$this->font_size);		
	    $aImg->StrokeText($x1+$this->mark_abs_size+$this->xmargin*1.5,$y1-1,$p[0]);
	    if( $this->layout==LEGEND_VERT )
