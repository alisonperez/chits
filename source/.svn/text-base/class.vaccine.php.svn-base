...
class vaccine extends module {
    ...
    function display_vaccine_record() {
        ...
        $patient_id = healthcenter::get_patient_id($get_vars["consult_id"]);
        $sql = "select vaccine_id, vaccine_timestamp, date_format(vaccine_timestamp,'%a %d %b %Y') ".
               "from m_consult_vaccine ".
               "where patient_id = '$patient_id' order by vaccine_id, vaccine_timestamp desc";
        if ($result = mysql_query($sql)) {
            if (mysql_num_rows($result)) {
                while (list($vaccine, $vstamp, $vdate) = mysql_fetch_array($result)) {
                    print "<img src='../images/arrow_redwhite.gif' border='0'/> ";
                    print "<a href='".$_SERVER["PHP_SELF"]."?page=".$get_vars["page"]."&menu_id=".$get_vars["menu_id"]."&consult_id=".$get_vars["consult_id"]."&ptmenu=".$get_vars["ptmenu"]."&module=vaccine&vaccine=$vaccine&ts=$vstamp#detail'>$vaccine</a> $vdate<br/>";
                    if ($get_vars["vaccine"]==$vaccine && $get_vars["ts"]==$vstamp) {
    ...
    function display_vaccine_record_details() {
        ...
        if ($result = mysql_query($sql)) {
            if (mysql_num_rows($result)) {
                ...
                print "<form name='form_vaccine_detail' method='post' action='".$_SERVER["PHP_SELF"]."?page=".$get_vars["page"]."&menu_id=".$get_vars["menu_id"]."&consult_id=".$get_vars["consult_id"]."&ptmenu=".$get_vars["ptmenu"]."&module=vaccine&vaccine=$vid&ts=$vstamp'>";
